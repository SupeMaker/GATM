...
class TextCNNClassifyModel(BaseClassifyModel):
    ...
    def conv_and_pool(self, x, conv):
        x = F.relu(conv(x)) #(32,100,255,1)
        x = x.squeeze(3) # (32,100,255)
        x = F.max_pool1d(x, x.size(2)) #(32,100,1)
        x = x.squeeze(2)
        return x

    def forward(self, input_feat, **kwargs):
        x = self.embedding_layer(input_feat).unsqueeze(1) # （32，1，256，300）
        x = torch.cat([self.conv_and_pool(x, conv) for conv in self.conv_layers], 1) #（32，100）
        x = nn.Dropout(self.dropout_rate)(x) #（32，100）
